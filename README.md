...
| 额度卡片 | 每个账号一组:窗口各一张卡片,**剩余**百分比(官方页面显示的是已用额度)、彩色进度条与重置倒计时;60s 刷新,倒计时每秒自走。多个账号纵向堆叠,每组带自己的名字 |
| 额度汇总行 | 卡片上方一行:`$31.0 可用额度 · 639.6M tokens · 5,218 runs`,即页面 Overview 的可用额度、本期总 token 与总请求数(仅 Command Code 有这三项) |
...
两个账号各占一组卡片,纵向堆叠,组名标在上方;没有数据的那组自动消失,不留空位。
两组的三个窗口语义相同(短窗口 / 周 / 月),卡片样式一致。

| 来源 | 需要什么 | 数据 |
|---|---|---|
| **Command Code** | 无需配置 —— 复用 zcode 里已配好的该 provider 及其密钥 | 5 小时 / 周 / 月剩余 + 可用额度、本期 token 数与请求数 |
| **OpenCode Go** | 需在 `~/.zcode/tps.json` 里填 `opencode.api_key`(**可选、默认关闭**) | Rolling(5 小时)/ 周 / 月剩余 + 各自重置时间 |

OpenCode Go 的额度要手动开启 —— 它不是 zcode 的 provider,密钥无法从 zcode 配置里读到。
在 `~/.zcode/tps.json` 里加上(与已有字段并列):

```json
{
  "opencode": { "api_key": "<你的 OpenCode API key>", "org_id": "<org_id>" }
}
```

`org_id` 就是控制台网址里那段(`opencode.ai/console/<org_id>/go`),`api_key` 从
`opencode.ai/auth` 获取(与控制台/CLI 同一套凭据)。可选的 `api_base` 用于改动接口地址。
**不填 `api_key` 时这个来源完全不启用,一次网络请求都不会发。**

> ⚠️ **OpenCode Go 这一路未经实测**:接口(`api.opencode.ai/orgs/<org>/go/status`)与字段名
> 是从控制台前端代码里反解出来的,响应结构为 `meters.{fiveHour,week,month}` 各自带
> `usedMicroCents / limitMicroCents`,月度重置取 `endsAt`;但这些是未公开路由,而本机当时没有
> 可用的 OpenCode 凭据,因此**没有用真实账号跑通过**。字段解析对意外情况一律保守处理:
> 任何结构不符就整组隐藏,不会显示错数字。若填好 key 后不出现,请把该接口的原始响应发我。
> Command Code 那一路是实测过的。
...
- **额度监控(Command Code + OpenCode Go)** —— 每个账号一组卡片,展示各窗口的剩余额度、
  彩色进度条与重置倒计时(官方页面显示的是已用,这里换算成剩余),额度不足时无需再开浏览器;
  多账号纵向堆叠,无数据的一组自动隐藏。Command Code 免配置,OpenCode Go 需在 `tps.json`
  里填 key(见"两个额度来源")
...
- **额度数据来源**:只读 GET,不写盘、不发送任何本地数据,失败时静默隐藏该组,不影响速度
  监控。Command Code 读 `api.commandcode.ai/alpha/*`(官方 CLI `/usage` 与网页同款接口),
  复用 `~/.zcode/v2/config.json` 里已配置的 provider 与其 API key;OpenCode Go 读
  `api.opencode.ai/orgs/<org>/go/status`(控制台同款接口),key 来自 `~/.zcode/tps.json`。
  两者都是未公开路由,官方改动会让对应那组消失而不是报错。
...
A:按来源分别判断。**Command Code** 需要 zcode 里配了该 provider 且密钥可用;没有订阅、
额度用尽或窗口限额为 `null` 时也整组隐藏(没有可显示的东西)。**OpenCode Go** 需要你在
`tps.json` 里填了 `opencode.api_key`,没填就完全不会请求。任一来源的多级失败处理一致:
没配 provider、没填 key、或密钥被拒绝(401/403)时立刻隐藏该组,不占空间也不报错;网络抖动
或官方接口临时异常则保留上一次读数,超过 5 分钟取不到新数据才隐藏——所以不会显示过期或
归零的错误数字。任一来源都不影响另一个,也不影响速度监控。数字 60 秒刷新一次(重置倒计时
每秒自走,不需要重新请求)。排查见 `~/.zcode/tps.log`。
